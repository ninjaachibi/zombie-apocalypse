...
void check_zombie_collision(struct Robot *robot_info, const double robot_x, const double robot_z)
{
  ...
  for (int i = 1; i <=n_zombies; i++)
  {
      ...
      char str2[10];
	    ...
	    sprintf(str2, "%d", i);
      //printf("ok");
	    char* z = concat(str1, str2);
	    
	    WbNodeRef zombie = wb_supervisor_node_get_from_def(z);
	    WbFieldRef zombie_trans_field = wb_supervisor_node_get_field(zombie, "translation");
	    const double *zombie_trans = wb_supervisor_field_get_sf_vec3f(zombie_trans_field);
	  
	    double zombie_x = zombie_trans[0];
	    double zombie_z = zombie_trans[2];
...
void check_zombie_collision2(struct Robot *robot_info, const double robot_x, const double robot_z)
{
